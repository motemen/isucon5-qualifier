...
#   }
    ...
    for (my $offset = 0; ;$offset += 50) {
        my $comments_data = db->select_all("SELECT * FROM comments ORDER BY created_at DESC LIMIT $offset, 50");
...
get '/diary/entries/:account_name' => [qw(set_global authenticated)] => sub {
    ...
    for my $entry (@$entry_list) {
        ...
        $entry->{comment_count} = $entry_count{ $entry->{id} } // 0;
        push @$entries, $entry;
    }
    mark_footprint($owner->{id});
    my $locals = {
